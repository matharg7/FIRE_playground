#!/bin/bash
# Push finished offline W&B runs to wandb.ai. Only needed when WANDB_MODE=offline.
# RUN THIS WHERE THERE IS INTERNET (a login node on Alliance clusters):
# compute nodes have no internet, and neither do dependent SLURM jobs, so a run
# cannot sync itself.
#
#   ./sync_wandb.sh                    # sync every finished, unsynced run
#   ./sync_wandb.sh --dry-run          # list what would be synced
#   ./sync_wandb.sh --root <dir>       # default: $FIRE_OUTPUT_DIR
#
# A run is eligible when train_sparse.py has written a DONE marker in its output
# directory and no SYNCED marker exists yet, so this is safe to re-run: each run
...
# Tests set FIRE_SYNC_NO_ENV=1 and point WANDB_BIN at a stub.
if [[ -z "${FIRE_SYNC_NO_ENV:-}" ]]; then
    fire_load_modules
    fire_activate || true
fi
