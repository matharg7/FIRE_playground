...
from sari import compute_sari  # replaces datasets.load_metric("sari"), removed upstream
...
def caculate_sari(inputs, results, data):
    # Upstream returned a 1-tuple (stray trailing comma) around this dict;
    # return the dict itself.
    return compute_sari(sources=inputs, predictions=results, references=[[label] for label in data])
