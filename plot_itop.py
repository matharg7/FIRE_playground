...
CSV_ROOT   = os.path.join(BASE_DIR, "csv_export","itop")
PLOT_DIR   = os.path.join(BASE_DIR, "plotter", "plots", "itop")
...
# X_LIM = (700, 1000) #zoom
X_LIM = (0, 1000) #reg
Y_LIM = (0, 1.025) #reg cifar10
# Y_LIM = (0, 0.33) #reg cifar100
#Y_LIM = (0.76,0.83) #zoom cifar10
# Y_LIM = (0.21,0.33) #zoom cifar100

sparsity_list = [0.1,0.3, 0.7,0.95,0.99]
    ...
    for seed in SEEDS:
        ...
        if os.path.isfile(fp):
            if os.path.getsize(fp) == 0:
                print(f"Warning: {fp} is empty. Skipping.")
                continue
            df = pd.read_csv(fp)
...
def main_grid(task="CIFAR100", pruning_ratio=0.3,
    ...
    #     f"{task}_grid_pr{pruning_ratio}_condensed.svg"), bbox_inches="tight", dpi=150)
    # plt.savefig(os.path.join(PLOT_DIR,
    #     f"{task}_grid_pr{pruning_ratio}.png"), bbox_inches="tight", dpi=150)
    plt.savefig(os.path.join(PLOT_DIR,
        f"{task}_grid.svg"), bbox_inches="tight", dpi=150)
    plt.savefig(os.path.join(PLOT_DIR,
        f"{task}_grid.png"), bbox_inches="tight", dpi=150)
    plt.close()
