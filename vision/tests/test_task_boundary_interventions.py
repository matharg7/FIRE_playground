...
@pytest.mark.parametrize("grow_init", ["zero", "stale"])
...
def test_stale_values_are_not_zero():
    model, opt, sp = _sparse_model(grow_init="stale")
    grown_values = []
    before = _run_until_mask_update(model, opt, sp)
    for name, w, mask in _sparse_layers(model):
...
def test_unknown_grow_init_raises():
    ...
    with pytest.raises(ValueError):
        _sparse_model(grow_init="something_else")


@pytest.mark.parametrize("sparsifier", ["gmp", "static"])
def test_stale_needs_a_regrowing_sparsifier(sparsifier):
    with pytest.raises(ValueError):
        _sparse_model(sparsifier=sparsifier, grow_init="stale")
